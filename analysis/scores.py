...
def calc_h2o_breakthrough_time_for_shell_size(x_cs,h2o_ads, h2o_diff, threshold, delta_t):
    v =  find_breakthrough_time(x_cs, h2o_ads, h2o_diff, threshold) - delta_t
    print(" bt_time - %s= " % delta_t, v)
    return v


def find_shell_size_for_breakthrough_time(*args):
    print("----")
    print("find_shell_size: ", *args)
    try:
        return root_scalar(calc_h2o_breakthrough_time_for_shell_size, args, bracket=(1e-5, 0.3)).root
    except ValueError:
        print("error")
        return 0.0
...
def find_total_adsorption(t, ads, diff, shell_size):
    ...
    print("find_total_adsorption, x_all: ", x_all)
    h2o_conc = np.array([conc(t, x, ads, diff, 0, 2*shell_size) for x in x_all])
    print("find_total_adsorption, h2o_conc per x: ", h2o_conc)
    return np.trapz(h2o_conc, x_all)
...
# define the breakthrough limit to be 1% of the equilibrium loading of co2 in UIO-67
# mofs['co2_breakthrough_limit'] = mofs[mofs.mof == "UIO-67"]['a_co2_mole_cm3'].values[0] / 100




print(mofs.mof)
# mofs = mofs[mofs.mof=="UIO-67"]

# find shell size s.t ∆t = 100s
process_time = 100 # seconds
mofs['shell_size_cm'] = [find_shell_size_for_breakthrough_time(*args, process_time) for args in
    mofs[['a_h2o_mole_cm3', 'd_h2o_cm2_s', 'co2_breakthrough_limit']].itertuples(index=False)]
...
cs['cs_n2_mole'] = cs['a_n2_mole_cm3_shell'] * cs['shell_volume_cm3'] + cs['a_n2_mole_cm3_core'] * cs['core_volume_cm3']
cs['cs_h2o_mole'] = cs['h2o_breakthrough_adsorption_mol_cm2'] * 4 * math.pi * cs['core_radius_cm'] ** 2 # this will underestimate for smaller spheres
...
cs.to_csv("scores-out-spheres-noco2shell-altscores.csv")
