...
@click.option("--output-molecule-plots", default=True, help="output plot per molecule")
@click.option("--fs-per-row", default=10, type=int, help="femtoseconds per row. Defaults to 10.")
@click.option("--average-rows", default=1, type=int, help="# of rows to average together to get a dataset of reasonable size (typically about 1000 points for graphing)")
@click.option("--max-molecules", default=0, type=int, help="maximum number of molecules to process. Useful for debugging")
def process_trajectories(filename, gas="", mof="", output_path="", output_molecule_plots=True, fs_per_row=10, average_rows=1, max_molecules=0):
    """
    process filename (should be a numpy array of x, y, z coords)
    """
    ### load data and truncate to multiple of average_rows
    data = np.load(filename) # row:molecule:x,y,z
    num_rows, num_molecules, num_cols = data.shape
    reduced_rows = num_rows // average_rows
    num_rows = reduced_rows * average_rows
    data = data[0:num_rows, :, :]
    if max_molecules > 0:
        data = data[:, 0:max_molecules, :]
        num_molecules = max_molecules

    # take row index for all rows, average average_rows number of them together, and convert to ns
    simple_t = np.mean(np.arange(0,num_rows).reshape(-1, average_rows), axis=1) * fs_per_row / 1e6
    ...
    for m in range(num_molecules):
        ...
        simple_results[m,:] = np.mean(results.reshape(-1, average_rows), axis=1)
    ...
    with open("diff_stats.yaml", "w") as f:
        yaml.dump(dict(
            ...
            msd_a2=float(msd),
            d_msd_a2_fs=float(d_msd),
            d_fit_a2_fs=float(d_fit),
            d_fit_lower_interval_a2_fs=float(d_lower_conf_bound),
            d_fit_upper_interval_a2_fs=float(d_upper_conf_bound)
        ), f, sort_keys=False)
    ...
    ax.hist(diffs, 41, density=False, facecolor='g')
