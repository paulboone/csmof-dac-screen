...
def d_selectivity(csv_path, outputpath="diff-selectivity.png", plotnum=0):
    ...
    if plotnum == 0:
        ...
        ax.annotate("0", (-.1,.1), size=5, xytext=(5,-2), textcoords='offset points',
            horizontalalignment="left", verticalalignment='top', fontsize=fsl, c=".5")
        ax.axline([1,0], slope=-1, ls="--", color=".8", zorder=1)
        ax.annotate("+1", (0.9,.1), size=5, xytext=(5,-2), textcoords='offset points',
            horizontalalignment="left", verticalalignment='top', fontsize=fsl, c=".5")
        ax.axline([2,0], slope=-1, ls="--", color=".8", zorder=1)
        ax.annotate("+2", (1.9,.1), size=5, xytext=(5,-2), textcoords='offset points',
            horizontalalignment="left", verticalalignment='top', fontsize=fsl, c=".5")
        ...
        for i, mof in enumerate(data.mof):
            ...
            if mof in ["UIO-67"]:
                ax.annotate(moflabels[i], (x, y), size=5, textcoords='offset points', xytext=(x-4.,y - 0), horizontalalignment='right', verticalalignment='center', fontsize=fs)
            elif mof in ["UIO-66", "UIO-66 2x OH", "UIO-66 OH", "UIO-66 4x F", "UIO-67 2x N$_3$", "UIO-67 2x alkane-OC$_3$"]:
                ax.annotate(moflabels[i], (x, y), size=5, textcoords='offset points', xytext=(x+4,y + 0), horizontalalignment='left', verticalalignment='center', fontsize=fsl)
        ...
        ax.axline([1,0], slope=-1, ls="--", color=".8", zorder=1)
        ax.annotate("+1", (1.1,-0.1), size=5, xytext=(5,-2), textcoords='offset points',
            horizontalalignment="left", verticalalignment='top', fontsize=fsl, c=".5")
        ax.axline([2,0], slope=-1, ls="--", color=".8", zorder=1)
        ax.annotate("+2", (1.9,.1), size=5, xytext=(5,-2), textcoords='offset points',
            horizontalalignment="left", verticalalignment='top', fontsize=fsl, c=".5")
        ...
        for i, mof in enumerate(data.mof):
            ...
            if (1.1 < x < 3.3) and (-0.6 < y < 0.1):
                if mof in ["UIO-67", "UIO-67 2x alkane-OC$_4$", "UIO-67 branched-HNC$_5$"]:
                    ax.annotate(moflabels[i] + " - %3.1f" % (float(data.CO2[i]) * 1e4), (x, y), size=5, xytext=(x-0.03,y - 0.), horizontalalignment='right', verticalalignment='center', fontsize=fsl)
                else:
                    ax.annotate(moflabels[i] + " - %3.1f" % (float(data.CO2[i]) * 1e4), (x, y), size=5, xytext=(x+0.03,y - 0.), horizontalalignment='left', verticalalignment='center', fontsize=fsl)
